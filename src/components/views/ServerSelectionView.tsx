...
const ServerSelectionView: React.FC<ServerSelectionViewProps> = ({ onConnect, isLoading, error }) => {
  ...
  return (
    <div className="max-w-2xl mx-auto mt-10 p-8 bg-gray-800 rounded-lg shadow-xl">
      ...
      <form onSubmit={handleSubmit} className="space-y-6" aria-labelledby="server-selection-title">
        <Input
          ...
          disabled={isLoading}
          aria-describedby={validationError && ipAddress ? "ip-validation-error" : undefined}
        />
        <Input
          label="Port Number"
          id="port"
          ...
          disabled={isLoading}
          aria-describedby={validationError && port ? "port-validation-error" : undefined}
        />
        {validationError && <p id="ip-validation-error" className="text-sm text-red-400" role="alert">{validationError}</p>}
        {error && <p className="text-sm text-red-400" role="alert">Connection failed: {error}</p>}
